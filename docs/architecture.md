...
- `CanvasWidget` is a renderer-neutral interface over OpenGL and raster
  ...
  unrelated messages are replayed unchanged. The first successful top-level
  window mapping uses the same bounded capture because Qt's Wayland client
  integration is initialized lazily: a raster canvas can still make its
  available Wayland-EGL plugin try Zink before Qt falls back to shared memory.
  Qt 6.2's unsupported activation request for a newly mapped fullscreen window
  is prevented with `WA_ShowWithoutActivating`; the compositor already owns the
  focus decision. Failed mappings replay every captured line.
...
| 24 | Linux is Wayland-only; OpenGL is probed with a real frame before `MainWindow`, with a shared-code raster canvas fallback; successful probe and first-map fallback noise is captured narrowly | WSLg and native compositors should use one standard path; XCB/software-GL environment heuristics hid driver failures and made GL context creation a launch requirement even though the shell currently paints `QImage`s; Qt initializes its Wayland client integration lazily even for raster windows | Linux distribution targets require X11 again, or tiled GL rendering makes raster parity prohibitively expensive |
